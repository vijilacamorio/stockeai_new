                        ...
                        if (debug) {
                            cLog(
                                'Error! One of your "show rows" options is not a number.'
                                    ...
                                    var selectedRowsPerPage = localStorage.getItem('rowsPerPage');

// If a value exists, set it as the default selection; otherwise, use a default value
if (selectedRowsPerPage !== null) {
    $('#numrows').val(selectedRowsPerPage);
} else {
    // Set a default value (e.g., 5 rows per page)
...
if (selectedRowsPerPage !== null) {
    ...
    $('#numrows').val(selectedRowsPerPage);
                  ...
                  numPerPage = parseInt($(this).val(), 10);
                     currentPage = 0;
        generatePaginationValues();
          // localStorage.setItem('rowsPerPage', numPerPage);
  ...
  //  const storedRowsPerPage = localStorage.getItem('rowsPerPage');
   ...
   // numPerPage = storedRowsPerPage !== null ? parseInt(storedRowsPerPage, 10) : 100;
    
    $("select#numrows").val(numPerPage);
