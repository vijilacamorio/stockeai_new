   ...
   <section class="content">
      ...
      <div class="col-sm-16 col-md-14">
            <div class="panel panel-bd lobidrag" style='height:80px; border: 3px solid #d7d4d6;'>
                <div class='col-sm-8'>
                <form id="fetch_tax" action="<?php echo base_url(); ?>Chrm/federal_tax_report" method="POST">
    ...
    <thead class="sortableTable">
     ...
     <th class="4 value" data-col="4" style="text-align:center; width: 300px;"><?php echo 'Pay Period'?></th>
            <th class="5 value" data-col="5" style="text-align:center;width: 200px;"><?php echo ('Federal Income Tax')?></th>
...
?>
        <tr>
            ...
            <td><?php echo number_format($f_tax['f_ftax'], 2); ?></td>
...
?>
 ...
 $i=0;
  $index = 0;
    foreach($fed_tax as $f_tax) {
         $s_stax_emplr = isset($fed_tax_emplr[$index]['f_ftax']) ? $fed_tax_emplr[$index]['f_ftax'] : 0;
        $employeeContributionTotal += $f_tax['f_ftax'];
...
$('#fetch_tax').submit(function (event) {
    ...
    $('#d_range').html("<p style='text-align:center;font-weight:bold;'>FROM : "+data[0].start+" TO : "+data[0].end+"</p>");
        ...
        data.forEach(function(item) {
          ...
          totalSStax += parseFloat(item.f_ftax);
          var row = "<tr>";
            row += "<td style='text-align: center;'>" + i + "</td>";
            row += "<td style='text-align: center;'>" + item.first_name ." ".item.middle_name." ".item.last_name + "</td>";
            ...
            row += "<td style='text-align: center;'>" +item.f_ftax + "</td>"; // Add currency symbol here
         ...
         var footerRow = "<tr  class='btnclr'>";
            footerRow += "<td style='text-align: end;' colspan='5'>Total </td>"; // Adjust colspan based on your table structure
