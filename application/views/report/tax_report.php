...
<?php
   ...
   $tax_inf = $CI->Accounts_model->tax_information_report();
...
<div class="content-wrapper">
   <section class="content-header"  style="height: 74px;">
      <div class="header-icon">
         ...
         <figure class="one">
               <img src="<?php echo base_url()  ?>asset/images/taxsable.png"  class="headshotphoto" style="height:50px;" />
      </div>

          <div class="header-title">
          <div class="logo-holder logo-9">
          <h1 style="margin-top: 11px;"><strong><?php echo 'Taxable Sales Details' ?><strong></h1>
       </div>
         ...
         <ol class="breadcrumb"   style=" border: 3px solid #d7d4d6;"   >
            ...
            <li class="active" style="color:orange;"><?php echo 'Taxable Sales' ?></li>
                    ...
                    <div class="sortableTable__discard">
                     ...
                     <div id="for_filter_by" class="for_filter_by" style="display: inline;">
                        ...
                        <select id="filterby" style="border-radius:5px;height:25px;" >
                           <option value="1"><?php echo 'Invoice No' ?></option>
                           <option value="2"><?php echo 'Tax' ?></option>
                           <option value="3"><?php echo 'Tax Agency' ?></option>
                        </select>
                        ...
                        <table class="table table-bordered" cellspacing="0" width="100%" id="ProfarmaInvList">
                           <thead class="sortableTable">
                              <tr  class="sortableTable__header btnclr">
                                 ...
                                 <th class="1 value" style='color:white;' data-col="1">Invoice NO</th>
                                 <th  class="4 value" style='color:white;'  data-col="4"><?php echo 'Total Amount' ?></th>
                                 <th class="2 value"  style='color:white;' data-col="2"><?php echo 'Tax' ?></th>
                                 <th class="3 value" style='color:white;' data-col="3"><?php echo 'Tax Agency' ?></th>
                                 <th  class="5 value" style='color:white;' data-col="5"><?php echo 'Tax Description' ?></th>
                              </tr>
                           </thead>
                           <tbody class="sortableTable__body" id="tab">
                              <?php
                                 ...
                                 // print_r($tax_inf);
                                 if ($sales_tax) {
                                     foreach ($sales_tax as $product) {
                                         // Extract the tax_id from the total_tax field
                                  $product['total_tax']=       preg_match('/\((.*?)\)/', $product['total_tax'], $matches);
                                         $tax_i = trim($matches[1]);
                                         $tax_id='';
                                 $tax_parts = explode('-', $tax_i);
                                 if (count($tax_parts) > 1) {
                                 $tax_id = trim($tax_parts[0]);
                                 } else {
                                 // Handle cases where there's no hyphen in the total_tax string
                                 $tax_id = trim($product['total_tax']);
                                 }
                                         // Search for a matching tax_id in the $tax_inf array
                                         $tax_agency = ''; // Initialize it as an empty string
                                 
                                         foreach ($tax_inf as $taxInfo) {
                                          
                                            // echo "<br/>".$taxInfo['tax_id']."==".$tax_id;
                                             if (trim($taxInfo['tax_id']) == trim($tax_id)) {
                                                 $tax_agency = trim($taxInfo['tax_agency']);
                                                 // echo "<br/>".$taxInfo['tax_id']."==".$tax_agency;
                              ...
                              <tr class="task-list-row">
                                 ...
                                 <td data-col="1" style="text-align:center;" class="1"><?php echo $product['commercial_invoice_number']; ?></td>
                                 <td  data-col="4" class="4" style="text-align:center; white-space:nowrap;"><?php echo $product['gtotal']; ?></td>
                                 <td data-col="2" class="2"><?php echo $tax_i; ?></td>
                                 <td data-col="3" class="3"><?php echo $tax_agency; ?></td>
                                 <td data-col="5" class="5" ><?php echo $taxInfo['description']; ?></td>
                              </tr>
                              <?php $i++;
                                 }
                                 } ?>
...
<script type="text/javascript">
         ...
         function storeVisibilityState() {
            var TaxableSalesDetailslistvisibilityStates = {};
            $("#ProfarmaInvList tr").each(function(index, element) {
                var row = $(element);
                var rowID = index;
                var isVisible = row.is(':visible');
                TaxableSalesDetailslistvisibilityStates[rowID] = isVisible;
            });
            // Store the visibility states in localStorage
            localStorage.setItem("TaxableSalesDetailslistvisibilityStates", JSON.stringify(TaxableSalesDetailslistvisibilityStates));
         }
         // Apply the stored visibility state on page load
         function applyVisibilityState() {
            var storedVisibilityStates = JSON.parse(localStorage.getItem("TaxableSalesDetailslistvisibilityStates")) || {};
            $("#ProfarmaInvList tr").each(function(index, element) {
                var row = $(element);
                var rowID = index;
