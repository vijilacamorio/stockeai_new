...
<div class="content-wrapper">
   <section class="content-header">
      <div class="header-icon">
         ...
         <ol class="breadcrumb"   style=" border: 3px solid #d7d4d6;"   >
            ...
            <li class="active" style="color:orange"><?php echo 'Unpaid Bills';?></li>
   ...
   <section class="content">
      ...
      <?php  
         ...
         foreach ($vendor_data['rows'] as $invoice) {
            ...
            <div class="panel panel-bd lobidrag" style='height:80px;border: 3px solid #d7d4d6;'>
               ...
               <table class="table" align="center">
                  <tr style='text-align:center;font-weight:bold;' class="filters">
                     ...
                     <td class="search_dropdown" style='width:300px;' >
                        <span><?php echo 'Vendor Name'; ?></span>
                        <select id="customer-name-filter"  style='width:300px;' class="form-control">
                     ...
                     <td><input type='submit' class="btn btnclr" style="color:white;color:white;margin-top: 15px;" value='submit' id='submit_btn'/></td>
                  </tr>
               </table></div>
                <div class='col-sm-2'>
                  <div class="dropdown bootcol" id="drop" style="    width: 300px;">
                     <button class="btnclr btn btn-default dropdown-toggle" type="button"  style=";margin-top: 20px;margin-left: 120px;float:left;color:white;" id="dropdownMenu1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                     ...
                     &nbsp;
                     <button type="button" style='color:white;'  class="btnclr btn btn-default dropdown-toggle"  onclick="printDiv('printableArea')"><b class="ti-printer"></b>&nbsp;<?php echo display('print') ?></button>
                  </div>
               </div>
            </div>
         </div>
                ...
                <table class="table table-bordered" cellspacing="0" width="100%" id="ProfarmaInvList" >
                  <thead class="sortableTable">
                     <tr  class="sortableTable__header btnclr">
                        <th class="1 value" data-col="1"    style="height: 45.0114px; width: 234.011px; text-align:center; " > <?php echo 'Bill No'?></th>
                        ...
                        <th class="4 value" data-col="4" style="width: 198.011px; text-align:center;"       ><?php echo 'Vendor Name ';?></th>
                        <th class="5 value" data-col="5" style="width: 198.011px; text-align:center;"       ><?php echo 'Payment Due Date';?></th>
                        <th class="6 value" data-col="6"  id="pastDueHeader"  style="width: 198.011px; text-align:center;"       ><?php echo 'Past Due';?></th>
                        ...
                        <th class="9 value" data-col="9" style="width: 198.011px; text-align:center;"       ><?php echo 'Status';?></th>
                     </tr>
                  </thead>
                  <tbody class="sortableTable__body" id="tab">
                     ...
                     <?php 
                        ...
                        ?>
                     <tr style="text-align:center" class="task-list-row" data-task-id="<?php echo $count; ?>" data-pname="<?php echo $arr['chalan_no'];  ?>" data-model="<?php echo $status; ?>" data-category="<?php echo $arr['supplier_name']; ?>" data-unit="<?php echo $arr['date'] ?>" data-supplier="<?php echo $arr['payment_due_date'];  ?>">
                        <!--<td style="display: none;"><input type="hidden" class="form-control" id="rowinvoice_id" value="<?php echo $arr['invoice_id'];  ?>" /></td>-->
                        <td data-col="1" class="1 value" style="text-align:center;">        <a   target="_blank" href="<?php if(!empty($arr['purchase_id'])) 
                           { 
                           ...
                           } ?>"><?php   echo $arr['chalan_no'];  ?></a>  </td>
                        <td data-col="2" class="2 value" style="text-align:center;" ><?php   echo $arr['purchase_date'];  ?></td>
                        <td data-col="3" class="3 value" style="text-align:center;"><?php  echo $currency; ?><?php   echo $arr['grand_total_amount'];  ?></td>
                        <td data-col="4" class="4 value" style="text-align:center;     text-wrap: nowrap;"><?php   echo $arr['supplier_name'];  ?></td>
                        ...
                        <td data-col="8" class="8 value" style="text-align:center;"><?php  echo $currency; ?><?php   echo $arr['balance'];  ?></td>
                        <td data-col="9" class="9 value" style="text-align:center;">
                           <?php echo $currency; ?>
                           <?php
                              if ($status == 'Paid') {
        ...
        function fnExcelReport()
{
 table = $('#ProfarmaInvList').clone();
   ...
   //     if (selectedPaymentFilter === 'custom') {
   ...
   //         }
   //     } else {
   //         // Hide the date picker for other options
   //         $('#datepicker-container').hide();
   //     }
   ...
   //             if (!selectedStartDate || !selectedEndDate) {
   //                 // If the date range is not selected, don't show the row
   //                 showRow = false;
   //             } else {
   //                 // If a date range is selected, compare with the paymentDueDate
   //                 if (paymentDueDate < selectedStartDate || paymentDueDate > selectedEndDate) {
   //                     showRow = false;
   //                 }
   //             }
   ...
   // Add an event listener for date picker changes
   // $(function() {
   //   $('input[name="daterangepicker-field"]').daterangepicker({
   //     opens: 'left'
   //   }, function(start, end, label) {
...
<script type="text/javascript">
         ...
         function storeVisibilityState() {
            var UnpaidInvoicesvisibilityStates = {};
            $("#ProfarmaInvList tr").each(function(index, element) {
                var row = $(element);
                var rowID = index;
                var isVisible = row.is(':visible');
                UnpaidInvoicesvisibilityStates[rowID] = isVisible;
            });
            // Store the visibility states in localStorage
            localStorage.setItem("UnpaidInvoicesvisibilityStates", JSON.stringify(UnpaidInvoicesvisibilityStates));
         }
         // Apply the stored visibility state on page load
         function applyVisibilityState() {
            var storedVisibilityStates = JSON.parse(localStorage.getItem("UnpaidInvoicesvisibilityStates")) || {};
            $("#ProfarmaInvList tr").each(function(index, element) {
                var row = $(element);
                var rowID = index;
