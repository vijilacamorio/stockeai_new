   ...
   <section class="content-header">
      <div class="header-icon">
         ...
         <ol class="breadcrumb"   style=" border: 3px solid #d7d4d6;"   >
            ...
            <li class="active" style="color:orange"><?php echo 'Sales Report';?></li>
               ...
               <table class="table table-bordered" cellspacing="0" width="100%" id="ProfarmaInvList">
                  <thead class="sortableTable">
                     <tr  class="sortableTable__header btnclr">
                        <th class="1 value" data-col="1" style="height: 45.0114px; text-align:center; width: 300px;"><?php echo 'Invoices ID'?></th>
                        <th class="2 value" data-col="2" style="text-align:center; width: 300px;"><?php echo 'Customer Name'?></th>
                  ...
                  <tbody class="sortableTable__body" id="tab">
                     <?php
                        $count=1;
                          if($get_salesdata){
                           ...
                           foreach ($get_salesdata as $key => $arr) {
                              // echo "<pre>";
                              // print_r($arr); 
                              // echo "</pre>";
                           $status = '';

                           if ($arr['gtotal'] == $arr['total_amountpaid']) {
                               $status = 'Paid';
                           } elseif ($arr['gtotal'] != $arr['total_amountpaid'] && $arr['total_amountpaid'] != '0.00') {
                               $status = 'Partially Paid';
                           } elseif ($arr['gtotal'] != $arr['total_amountpaid'] && $arr['total_amountpaid'] == '0.00') {
                               $status = 'Not Paid';
                           } elseif (substr($arr['due_amount'], 0, 1) == '-') {
                               $status = 'Overpaid';
                           }
                     ...
                     ?>
                        ...
                        ?>
                     <tr style="text-align:center" class="task-list-row" data-task-id="<?php echo $count; ?>" data-pname="<?php echo $arr['customer_name'];  ?>" data-category="<?php echo $arr['product_name']; ?>" data-supplier="<?php echo $arr['payment_due_date'] ?>" data-model="<?php echo $status; ?>">

                        <?php 
                         
                        if ($arr['invoice_id'] != $previousInvoiceID) {
                            echo '<td class="1 value" data-col="1" style="text-align: center;">' . $arr['invoice_id'] . '</td>';
                            $previousInvoiceID = $arr['invoice_id'];
                        } else {
                            echo '<td class="1 value" data-col="1"></td>';
                        }

                        if ($arr['customer_name'] != $previousCustomerName) {
                            echo '<td class="2 value" data-col="2" style="text-align: center;">' . $arr['customer_name'] . '</td>';
                            $previousCustomerName = $arr['customer_name'];
                        } else {
                        ...
                        <td class="4 value" data-col="4" style="text-align:center;"><?php echo $currency . $arr['gtotal']; ?></td>
                        <td class="5 value" data-col="5" style="text-align:center;"><?php echo $arr['payment_due_date']; ?></td>
                        <td class="6 value" data-col="6" style="text-align:center;"><?php if(!empty($arr['total_amountpaid'])){echo $arr['total_amountpaid'];}else{echo "0.00";} ?></td>
                        <td class="7 value" data-col="7" style="text-align:center;"><?php echo $arr['due_amount'] ? $currency . $arr['due_amount'] : '0.00'; ?></td>
