...
'Pacific/Guam' => 'Pacific/Guam',
       ...
       // Changed by Ajith
       public function invoice_setting($encodedId) {
        $CI = & get_instance();
        $CI->load->model('Web_settings');
        ...
        $curlist          = $CI->Web_settings->currency_list();
        $language         = $CI->Web_settings->languages();
        $sales_data       = $CI->Web_settings->get_data_sales($decodedId);
        $ocean_data       = $CI->Web_settings->get_data_ocean($decodedId);
        $truck_data       = $CI->Web_settings->get_data_roadtransport($decodedId);
        $data = array(
            'title'             => display('update_setting'),
            'logo'              => $setting_detail[0]['logo'],
            'invoice_logo'      => $setting_detail[0]['invoice_logo'],
...
public function email_setting($view_email, $email_con) {
    ...
    public function invoice_design($decodedId){
        $CI = & get_instance();
        $CD = & get_instance();
        ...
        $setting_detail = $CI->Web_settings->retrieve_setting_editdata($decodedId);
        $dataw = $CI->invoice_design->retrieve_data($decodedId);
        $datac = $CD->Companies->company_details($decodedId);
        $datacontent = $CI->invoice_content->retrieve_data($decodedId);
        $data= array(
        'header'=> (!empty($dataw[0]['header']) ? $dataw[0]['header'] : '') , 
        'logo'=> (!empty($setting_detail[0]['logo']) ? $setting_detail[0]['logo'] : '') , 
        'color'=> (!empty($dataw[0]['color']) ? $dataw[0]['color'] : '') ,   
        'invoice_logo' =>(!empty($setting_detail[0]['invoice_logo']) ? $setting_detail[0]['invoice_logo'] : '') ,   
        'cname'=>(!empty($datac[0]['company_name']) ? $datacontent[0]['company_name'] : '') ,    
        'address'=>(!empty($datac[0]['address']) ? $datacontent[0]['address'] : '') ,     
        'mobile'=>(!empty($datac[0]['mobile']) ? $datacontent[0]['mobile'] : '') ,  
        'email'=>(!empty($datac[0]['email']) ? $datacontent[0]['email'] : '') ,     
    ...
    ////////////////INvoice Content/////////////
     public function invoice_content($decodedId ,$encodedId) {
        ...
        'invoice_logo' =>(!empty($setting_detail[0]['invoice_logo']) ? $setting_detail[0]['invoice_logo'] : '') ,  
        'address'=>(!empty($datacontent[0]['address']) ? $datacontent[0]['address'] : '') ,     
        'cname'=>(!empty($datacontent[0]['company_name']) ? $datacontent[0]['company_name'] : '') ,    
        'phone'=>(!empty($datacontent[0]['mobile']) ? $datacontent[0]['mobile'] : '') ,  
