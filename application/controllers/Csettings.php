...
public function add_new_bank() {
    ...
    $headcode = ($coa && $coa->HeadCode != NULL) ? $coa->HeadCode + 1 : "102010201";
    $createby = $this->input->post('encodedId');
    $createdate = date('Y-m-d H:i:s');
    $data = array(
        'created_by' => $createby,
    ...
    $bankinfo = $this->Settings->bank_entry_new($data , $createby);
    if ($bankinfo) {
        $response = array(
            'status'   => 'success',
        ...
        $response = array(
            'status' => 'failure',
            'msg'    => 'Failed to add bank. Please try again.'
        );
    }
